...
async function handle(request, env, ctx) {
  ...
  if (path === "/health") {
    ...
    const dnsError = String(lastDnsSync?.error || "");
    const dnsRateLimited = lastDnsSync?.ok === false && /rate limit|rate limited|throttl/i.test(dnsError);
    const reasons = [];
    if (!ips.length) reasons.push("empty-pool");
    if (info.stale) reasons.push("stale");
    // Cloudflare DNS batch API may be temporarily rate limited. The IP pool is still fresh
